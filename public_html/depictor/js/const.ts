export const API_USER_AGENT:string = 'Depictor/1.0 (https://hay.toolforge.org/depictor/; hay@haykranen.nl)';
export const COMMONS_USER_PREFIX:string = 'https://commons.wikimedia.org/wiki/User:';
export const DEFAULT_LOCALE = 'en';
export const IMAGE_SIZE = 500; // https://www.mediawiki.org/wiki/Common_thumbnail_sizes
export const LOCAL_API_ENDPOINT = './api/index.php';
export const MAX_API_TRIES = 3;
export const MAX_API_CHECK_TRIES = 100;
export const MAX_BIRTH_YEAR = 2012;
export const MAX_CHALLENGE_OVERVIEW_COUNT = 5;
export const MAX_PRELOAD_BATCH = 50;
export const MIN_BIRTH_YEAR = 1880;
export const MIN_CANDIDATES_FOR_CHALLENGE = 100;
export const MIN_CHALLENGE_TITLE_LENGTH = 5;
export const MIN_CHALLENGE_SHORTDESCRIPTION_LENGTH = 25;
export const MIN_ITEMS_FOR_CHALLENGE = 10;
export const THUMB_SIZE = 330; // https://www.mediawiki.org/wiki/Common_thumbnail_sizes